...
pub enum CartridgeError {
    ...
    /// An error occurred while saving SRAM to file.
    #[error("SRAM save error: {0}")]
    SramError(String),
}
...
impl From<SramError> for CartridgeError {
    fn from(from: SramError) -> Self {
        Self::SramError(from.to_string())
    }
}

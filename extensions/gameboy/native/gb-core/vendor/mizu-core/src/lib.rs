...
impl GameBoy {
    ...
    /// Get battery-backed SRAM data, if present.
    pub fn get_sram(&self) -> Option<&[u8]> {
        self.bus.get_sram()
    }

    /// Get mutable battery-backed SRAM data, if present.
    pub fn get_sram_mut(&mut self) -> Option<&mut [u8]> {
        self.bus.get_sram_mut()
    }

    /// Save SRAM to the .sav file on disk.
    pub fn save_sram_to_file(&self) -> Result<(), CartridgeError> {
        self.bus.save_sram_to_file()
    }

...
impl Cartridge {
    ...
    pub fn has_battery(&self) -> bool {
        self.cartridge_type.battery
    }

    pub fn get_raw_ram(&self) -> &[u8] {
        &self.ram
    }

    pub fn get_raw_ram_mut(&mut self) -> &mut [u8] {
        &mut self.ram
    }

    pub fn save_sram_to_file(&self) -> Result<(), CartridgeError> {
        if self.cartridge_type.battery {
            self.save_sram_file().map_err(|e| CartridgeError::SramError(e.to_string()))?;
        }
        Ok(())
    }
}

...
import type { GameBoyCore } from "./gb-core.js";
import { saveState } from "./saves.js";
...
export interface GameBoySessionOptions {
	core: GameBoyCore;
	romPath: string;
	saveDir: string;
...
export interface GameBoySessionStats {
	tickFps: number;
	renderFps: number;
...
export class GameBoySession {
	...
	private stats: GameBoySessionStats;
	private statsWindow = {
		startTime: 0,
		frames: 0,
		renders: 0,
	...
	constructor(options: GameBoySessionOptions) {
	...
	private tick(): void {
		...
		try {
			...
			const intervalMs = this.fastForward ? this.frameIntervalMs / 2 : this.frameIntervalMs;
			const framesDue = Math.floor(this.accumulatedTime / intervalMs);
			if (framesDue <= 0) {
				return;
			}
			...
			if (framesDue > this.maxCatchUpFrames) {
				...
				this.accumulatedTime -= framesDue * intervalMs;
			}
			...
			if (!this.fatalErrorLogged) {
				...
				console.error(`Game Boy session crashed: ${message}`);
			}
			void this.stop();
		}
	}
	...
	private async saveIfNeeded(force: boolean): Promise<void> {
		...
		try {
			await saveState(this.saveDir, this.romPath, state);
		} catch (error) {
			if (!this.saveErrorLogged) {
				this.saveErrorLogged = true;
				const message = error instanceof Error ? error.message : String(error);
				console.warn(`Game Boy save failed: ${message}`);
			}
		} finally {

...
export function getStatePath(saveDir: string, romPath: string): string {
	const romName = getRomDisplayName(romPath);
	const hash = getSaveId(romPath);
	return path.join(saveDir, `${romName}-${hash}.state`);
}
...
export async function loadState(saveDir: string, romPath: string): Promise<Uint8Array | null> {
	const statePath = getStatePath(saveDir, romPath);
	try {
...
export async function saveState(saveDir: string, romPath: string, data: Uint8Array): Promise<void> {
	await fs.mkdir(saveDir, { recursive: true });
	const statePath = getStatePath(saveDir, romPath);

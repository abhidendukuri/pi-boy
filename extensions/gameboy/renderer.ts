...
import type { FrameBuffer } from "./gb-core.js";
...
export class GameBoyImageRenderer {
	...
	private readonly rawFilePath = path.join(this.rawFileDir, `pi-gb-tty-graphics-${this.imageId}.raw`);
	...
	private renderKittySharedMemory(
		...
		this.rawVersion += 1;
		const marker = `\x1b_pi:gb:${this.rawVersion}\x07`;
	...
	private handleRenderError(kind: string, error: unknown): string[] {
		if (!this.renderErrors.has(kind)) {
			...
			console.warn(`Game Boy renderer ${kind} failed: ${message}`);
		}

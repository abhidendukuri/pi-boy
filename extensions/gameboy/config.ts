...
export interface GameBoyConfig {
	romDir: string;
	saveDir: string;
	enableAudio: boolean;
...
const DEFAULT_ROM_DIR = path.join(os.homedir(), "roms", "gameboy");

export function getDefaultSaveDir(romDir: string): string {
	return path.join(romDir, "saves");
}

export const DEFAULT_CONFIG: GameBoyConfig = {
...
export function getConfigPath(): string {
	return path.join(os.homedir(), ".pi", "gameboy", "config.json");
}
...
export function normalizeConfig(raw: unknown): GameBoyConfig {
	...
	const imageQuality = raw === "high" ? "high" : "balanced";
	...
	return {
		...
		isDmg: typeof parsed.isDmg === "boolean" ? parsed.isDmg : DEFAULT_CONFIG.isDmg,
	};
}

export function formatConfig(config: GameBoyConfig): string {
	return JSON.stringify(config, null, 2);
}

export async function loadConfig(): Promise<GameBoyConfig> {
...
export async function saveConfig(config: GameBoyConfig): Promise<void> {
	const configPath = getConfigPath();
